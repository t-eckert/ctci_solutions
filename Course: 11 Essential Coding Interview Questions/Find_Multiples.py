    ...
    for i in range(len(input_list)):
        if product % input_list[i] == 0:
            validity = two_multiples(
                input_list[:i] + input_list[i + 1 :], product / input_list[i]
            )
            if validity == True:
                return True
    return False
...
for product in range(1, 100):
    for i in range(len(input_lists)):
        start_two = timer()
        print(
            "Two factors of %s in %s : %s"
            % (product, input_lists[i], two_multiples(input_lists[i], product))
        )
        end_two = timer()
        print("Process Time: %s s" % (end_two - start_two))
        start_three = timer()
        print(
            "Three factors of %s in %s : %s"
            % (product, input_lists[i], three_multiples(input_lists[i], product))
        )
        end_three = timer()
